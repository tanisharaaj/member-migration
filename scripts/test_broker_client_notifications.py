"""
scripts/test_broker_client_notifications.py

This script tests the full BrokerNotifyWorkflow:
1. Reads client IDs from a Google Sheet tab.
2. Sends broker notification emails (using Broker Template 1).
3. Waits 2 minutes.
4. Sends client notification emails (using Client Template 1).
5. Returns a BatchResult with ItemResults for each client (and email).

Usage:
    python scripts/test_broker_client_notifications.py
Make sure:
- Worker is running and registered with Temporal.
- DATA_API and SendGrid are configured correctly.
"""
...
async def main():
    ...
    workflow_id = f"broker-client-test-{uuid.uuid4().hex[:6]}"

    # 3. start workflow
    handle = await client.start_workflow(
        BrokerNotifyWorkflow.run,
        BatchInput(
            tab_name="Batch 1",  # update with your test tab
